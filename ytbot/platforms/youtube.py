...
class YouTubeHandler(PlatformHandler):
    ...
    def _load_youtube_cookies(self) -> Optional[str]:
        ...
        2. Load from project root .youtube_cookies.json file (browser extension format)
        3. Load from YOUTUBE_COOKIES_FILE environment variable
        4. Load from YOUTUBE_COOKIES_JSON environment variable (Netscape format)
        ...
        default_cookie_json = '.youtube_cookies.json'
        if os.path.exists(default_cookie_json):
            logger.info(f"Found YouTube cookies JSON file: {default_cookie_json}")
            temp_file = os.path.join(tempfile.gettempdir(), 'youtube_cookies.txt')
            if self._convert_json_cookies_to_netscape(default_cookie_json, temp_file):
                logger.info(f"Converted JSON cookies to Netscape format: {temp_file}")
                return temp_file
            else:
                logger.error("Failed to convert JSON cookies to Netscape format")
    ...
    def _convert_json_cookies_to_netscape(self, json_path: str, output_path: str) -> bool:
        """
        Convert JSON cookies (browser extension format) to Netscape format.

        Args:
            json_path: Path to JSON cookie file
            output_path: Path to output Netscape format file
        ...
        try:
            ...
            if not isinstance(cookies, list):
                logger.error("Invalid cookie JSON format: expected a list")
                return False

            with open(output_path, 'w', encoding='utf-8') as f:
                f.write("# Netscape HTTP Cookie File\n")
                f.write("# This file was automatically generated by YTBot\n")
                f.write("# Converted from .youtube_cookies.json\n\n")
                ...
                for cookie in cookies:
                    ...
                    domain_flag = 'TRUE' if not host_only else 'FALSE'
                    secure_flag = 'TRUE' if secure else 'FALSE'

                    cookie_line = (
                        f"{domain}\t{domain_flag}\t{path}\t"
                        f"{secure_flag}\t{int(expires)}\t{name}\t{value}\n"
                    )
                    f.write(cookie_line)
            ...
            logger.error(f"Failed to parse JSON cookies: {e}")
            return False
        except Exception as e:
    ...
    async def get_content_info(self, url: str) -> Optional[ContentInfo]:
        ...
        try:
            ...
            error_msg = str(e)
            youtube_error = self._parse_youtube_error(error_msg)
            logger.error(f"Failed to get YouTube content info for {url}: {youtube_error}")

            return ContentInfo(
                url=url,
                title='Unknown',
                error_detail=youtube_error
            )

    def _parse_youtube_error(self, error_msg: str) -> str:
        """Parse YouTube-specific error messages and return user-friendly messages"""
        error_lower = error_msg.lower()

        if 'sign in' in error_lower or 'please sign in' in error_lower:
            return "ERROR_SIGN_IN_REQUIRED"
        if 'age' in error_lower and ('restrict' in error_lower or 'limit' in error_lower):
        ...
        if 'removed' in error_lower:
            return "ERROR_VIDEO_REMOVED"
        if 'geo' in error_lower and ('restrict' in error_lower or 'block' in error_lower):
            return "ERROR_GEO_RESTRICTED"
        if 'payment' in error_lower or 'purchase' in error_lower:
    ...
    def get_error_message(self, error_code: str) -> str:
        ...
        error_messages = {
            ...
            "ERROR_GEO_RESTRICTED": (
                "🌍 地区限制\n\n"
                "此视频在您所在的地区不可用。"
            ),
            "ERROR_RENTAL_PURCHASE": (
                "💰 需要购买/租借\n\n"
                "此视频需要购买或租借才能观看。"
            ),
        ...
        return error_messages.get(error_code, error_messages["ERROR_GENERIC"])
