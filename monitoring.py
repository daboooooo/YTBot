...
async def check_telegram_connection():
    ...
    """
    try:
        # 使用TelegramCommunicator进行连接测试
        from telegram_communicator import TelegramCommunicator

        communicator = TelegramCommunicator()
        if await communicator.connect():
            await communicator.bot.get_me()
            await communicator.disconnect()
            logger.debug("Telegram连接检查通过")
            return True
        else:
            logger.error("无法创建TelegramCommunicator实例，Telegram连接可能有问题")
            return False
...
async def telegram_connection_monitor():
    """
    定期监控Telegram连接状态，确保连接正常
    """
    logger.info("Telegram连接监控任务已启动")
    ...
    while True:
        try:
            # 检查Telegram连接
            connection_status = await check_telegram_connection()

            # 如果连接状态发生变化，记录日志
            if connection_status != last_connection_status:
                if connection_status:
                    logger.info("Telegram连接已恢复")
                else:
                    logger.warning("Telegram连接已断开")

                last_connection_status = connection_status

        except Exception as e:
            logger.error(f"Telegram连接监控过程中出错: {str(e)}")
            import traceback
...
def setup_signal_handlers():
    ...
    def signal_handler(sig, frame):
        ...
        try:
            # 尝试获取处理中更新数
            from main import processing_updates
            logger.info(
                "关闭前 - 处理中更新数: %d",
